  ...
  <div>  
    ...
    <v-card class="mx-auto lighten-5 ma-10" max-width="1400px">
      <v-system-bar height="60" dark color="deep-purple lighten-1">
        ...
        <span class="headline">Vision AI - 이미지 분류</span>          
      </v-system-bar>
      <v-row no-gutters>
        <v-col cols="12">
        </v-col>
      ...
      <v-row class='ml-10 mr-10' dense>     
        ...
        <v-col cols="12" md="6">
          <v-row dense> 
            ...
            <v-col cols="12">
              <v-card-text class="text-h2 font-weight-bold blue--text"> 
                 {{ predicted_label }}
              </v-card-text>        
            </v-col>            
          </v-row>
  ...
  methods: {
    ...
    classification() {
      if (this.chosenFile) {
        ...
        let url = process.env.VUE_APP_API_SERVER + '/classifier/classification'
        ...
        axios.post(url, post_data).then((response) => {
          ...
          this.predicted_label = response.data.predicted_label
          this.$swal.fire({
            icon: response.data.info,
            title: response.data.predicted_label
          })               
        })
      } else {
        this.$swal.fire({
